...
ffmpeg -f lavfi -i rgbtestsrc \
  -vf "drawtext=fontfile=$FONTFILE: \
  fontsize=25: fontcolor=0xFFFFFF: text='%{pts\:hms}-----%{n}': box=1: boxcolor=0x000000@1" \
  -target ntsc-dv -frames:v 1000 03-rgbtestsrc-0000.dv
...
ffmpeg -f lavfi -i mandelbrot \
  -vf "drawtext=fontfile=$FONTFILE: \
  fontsize=25: fontcolor=0xFFFFFF: text='%{pts\:hms}-----%{n}': box=1: boxcolor=0x000000@1" \
  -target ntsc-dv -frames:v 1000 08-mandelbrot-0000.dv

ffmpeg -i 08-mandelbrot-0000.dv -vf lutrgb=g=0:b=0 -target ntsc-dv 08-mandelbrot-0001.dv

ffmpeg -i 08-mandelbrot-0000.dv -vf lutrgb=r=0:b=0 -target ntsc-dv 08-mandelbrot-0002.dv

ffmpeg -i 08-mandelbrot-0000.dv -vf lutrgb=r=0:g=0 -target ntsc-dv 08-mandelbrot-0003.dv
